...
import {
	BCard, BCardText, BRow, BCol, BFormSelect, BLink, BFormGroup, BFormInput, BInputGroupAppend, BInputGroup, BFormCheckbox, BCardTitle, BImg, BForm, BButton,
} from 'bootstrap-vue'
...
export default {
	...
	data() {
		return {
			...
			batchList: [ { value: '', text: 'Seleccione el lote colindante' }, { value: '25', text: 'Lote A' }],
			required
		}
	},
	methods: {
		...
		initBatchs(batchList) {
			this.batchList = this.batchList.concat(batchList
				.map(batch => ({ text: batch.name, value: batch.id })));
		},
		...
		async validationForm() {
			...
			}).finally(() => this.$refs.batchForm.reset() );
		},
